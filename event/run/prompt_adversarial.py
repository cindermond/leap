...
def eval(deriver, eval_set, tokenizer, device, prompt_length_enc, prompt_length_dec, leng):
    ...
    path = f'data/exclude_list/task2_exclude_table.jsonl'
    with jsonlines.open(path, 'r') as file:
        lines = [line for line in file.iter()]
        lines = lines[:1313]
        lines = [[i for i in line["exclude"]] for line in lines]
    ...
    with open(f'data/modified/trainset-task2-ultimate.pickle', 'wb') as handle:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--leng',default="657",type=str)
    parser.add_argument('--dataset_prefix', default="entailment-bank-task-2-tot", type=str)
