    ...
    def predict(batched_data, is_plan_, max_plan_para_piece_):
        ...
        for _ in range(min(max_step-max_context_length, max_length)):
            ...
            if is_plan_:
                ...
                #entailment step 1
                if plan_early_end:
                    inputs = tokenizer_deberta([(s + ".", batched_data[ind_of_theory]["hypothesis"]+".")  for ind_of_theory in range(num_batch) for s in generated_seq[batched_length_2[ind_of_theory]:batched_length_2[ind_of_theory+1]]], return_tensors='pt', padding=True)
                    input_ids = inputs['input_ids'].to(device)
                    attention_mask = inputs["attention_mask"].to(device)
            ...
            if is_plan_:
                ...
                for d in range(plan_depth):
                    ...
                    #entailment
                    if plan_early_end:
                        inputs = tokenizer_deberta([(s+".",batched_data[ind_of_theory]["hypothesis"]+".") for ind_of_theory in range(num_batch) for s in generated_seq[(batched_length[ind_of_theory]*num_beams_der):(batched_length[ind_of_theory+1]*num_beams_der)]], return_tensors='pt', padding=True)
                        input_ids = inputs['input_ids'].to(device)
                        attention_mask = inputs["attention_mask"].to(device)
                        token_type_ids = inputs["token_type_ids"].to(device)
                        current_pos = 0
            ...
            for ind_of_theory, (beam_of_one_theory, past_score_of_one_theory, buffer_of_one_theory, proof_score_of_one_theory) in enumerate(zip_equal(beams, beam_scores, beams_sel_buffer, proof_scores)):
                if current_num_beams[ind_of_theory] == 0:
                    continue
                inputs = tokenizer_deberta([(beam[-1]+".", batched_data[ind_of_theory]["hypothesis"]+".") for beam in beam_of_one_theory], return_tensors='pt', padding=True)
                input_ids = inputs['input_ids'].to(device)
                attention_mask = inputs["attention_mask"].to(device)
                token_type_ids = inputs["token_type_ids"].to(device)
        ...
        for ind_of_theory, (beam_of_one_theory, past_score_of_one_theory, buffer_of_one_theory, proof_score_of_one_theory) in enumerate(zip_equal(beams, beam_scores, beams_sel_buffer, proof_scores)):
            ...
            if len(ended_beam_scores[ind_of_theory])>0:
                for result_pos in range(len(ended_beam_scores[ind_of_theory])):
                    inputs = tokenizer_deberta(ended_beams[ind_of_theory][result_pos][-1]+".", batched_data[ind_of_theory]['hypothesis']+".", return_tensors='pt')
                    input_ids = inputs['input_ids'].to(device)
                    token_type_ids = inputs['token_type_ids'].to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_path_der', default="supervised-deriver-weight-best-e=500-s=42-l=0.1-e=2-b=16-m=t5-small-p=4-p=4-d=entailment-bank-task-1-inf-", type=str)   #parser.add_argument('--load_path_der', default="supervised-deriver-weight-best-e=500-s=42-l=0.1-e=2-b=16-m=t5-small-p=4-p=4-d=entailment-bank-task-1-inf-", type=str)
